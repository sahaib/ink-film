...
/* Sound, synthesised in the browser — off until the viewer turns it on.
   ...
   What a plate declares (both optional):
     sfx:  [{ at: 1.7, kind: 'thunder' }]   at = plate-local seconds; kind = a key of `sfx` below:
           chime heart snap tick stamp coin thunder crackle whoosh hiss
     mood: (t) => ({ drone, warm, rumble, rain, wind })   each 0..1, a missing bed is 0.
           Without mood, a paper-toned moment gets { drone: 0.35, warm: 0.5 } and a void one
           { drone: 0.7, warm: 0.15 }. */
(function () {
  ...
  FILM.audio = {
    ...
    renderOffline({ rate = 48000 } = {}) {
      ...
      // every cue at its place on the timeline (live playback skips an unknown kind silently; the
      // export says so)
      for (const s of FILM.ORDER) {
        const sc = FILM.scenes[s.id];
        if (!sc || !sc.sfx) continue;
        for (const c of sc.sfx) {
          if (e.sfx[c.kind]) e.sfx[c.kind](s.start + c.at, c);
          else console.warn(`[ink-film] ${s.id}: no sound "${c.kind}"; kinds are ${Object.keys(e.sfx).join(' ')}`);
        }
      }
